...
export default function MovieDetails() {
  ...
  const { isFetching, error, data: movie = { lists: [] }, isUninitialized } = useGetMovieDetailsQuery({ id: dialog.params.id, timestamp: dialog.params.timestamp }, {
    skip: !(dialog.params && dialog.params.id),
  });
  const { isFetching: creditsIsFetched, error: creditsError, data: credits, isUninitialized: creditsIsUninitialized } = useGetMovieCreditsQuery({ id: dialog.params.id, timestamp: dialog.params.timestamp }, {
    skip: !(dialog.params && dialog.params.id),
          ...
          >
            <div className="inline-block relative w-full max-w-lg my-8 text-left align-middle transition-all transform border border-main-light bg-main-dark shadow-xl rounded-xl">
              ...
              <div className="mt-3 flex">
                <button disabled={isFetching} title="favorite" className={`rounded-bl-xl group relative flex-1 mr-0.5 flex justify-center items-center py-3 border-t-4 ${authenticated ? (movie.lists.includes('fav') ? 'bg-danger border-danger-dark' : 'bg-main border-danger') : 'bg-main-dark border-danger-dark cursor-not-allowed'}`} onClick={(e) => { btnClicked(e, 'fav', movie.id) }}>
                  {
                    !authenticated ? (
                ...
                <button disabled={isFetching} title="watch later" className={`group relative flex-1 mr-0.5 flex justify-center items-center py-3 border-t-4 ${authenticated ? (movie.lists.includes('later') ? 'bg-success border-success-dark' : 'bg-main border-success') : 'bg-main-dark border-success-dark cursor-not-allowed'}`} onClick={(e) => { btnClicked(e, 'later', movie.id) }}>
                  {
                    !authenticated ? (
                      <span className="hidden group-hover:inline-block"><Tooltip width="250px" title="You're not signed in" body="Please sign in to add movies to your list!" /></span>
                    ) : (undefined)
                ...
                <button disabled={isFetching} title="wish list" className={`group relative flex-1 mr-0.5 flex justify-center items-center py-3 border-t-4 ${authenticated ? (movie.lists.includes('wish') ? 'bg-warning border-warning-dark' : 'bg-main border-warning') : 'bg-main-dark border-warning-dark cursor-not-allowed'}`} onClick={(e) => { btnClicked(e, 'wish', movie.id) }}>
                  {
                    !authenticated ? (
                      <span className="hidden group-hover:inline-block"><Tooltip width="250px" title="You're not signed in" body="Please sign in to add movies to your list!" /></span>
                    ) : (undefined)
                ...
                <button disabled={isFetching} title="own it" className={`rounded-br-xl group relative flex-1 flex justify-center items-center py-3 border-t-4 ${authenticated ? (movie.lists.includes('own') ? 'bg-primary border-primary-dark' : 'bg-main border-primary') : 'bg-main-dark border-primary-dark cursor-not-allowed'}`} onClick={(e) => { btnClicked(e, 'own', movie.id) }}>
                  {
                    !authenticated ? (
                      <span className="hidden group-hover:inline-block"><Tooltip width="250px" title="You're not signed in" body="Please sign in to add movies to your list!" /></span>
                    ) : (undefined)
